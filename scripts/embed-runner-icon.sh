...
# instead of a blank icon. It also adds the modern launch-screen declaration
# that prevents iOS from opening XCTRunner in the legacy 320x480 letterbox.
#
...
# XCTRunner.app ships without launch-screen metadata. On modern iPhones that
# makes UIKit select the legacy compatibility canvas, leaving black bars above
# and below the app. An empty UILaunchScreen dictionary opts into native sizing
# without requiring a storyboard inside Apple's generated test host.
...
if [ -d "$RUNNER_APP/_CodeSignature" ]; then
    ...
    if [ -z "$EXISTING_IDENT" ]; then
        # Scheme post-actions do not inherit CODE_SIGN_* variables. Resolve the
        # exact certificate used by the embedded provisioning profile so
        # duplicate certificates with the same display name are unambiguous.
        PROFILE_PLIST=$(mktemp)
        PROFILE_CERT=$(mktemp)
        if security cms -D -i "$RUNNER_APP/embedded.mobileprovision" -o "$PROFILE_PLIST" 2>/dev/null \
            && /usr/libexec/PlistBuddy -c 'Print :DeveloperCertificates:0' "$PROFILE_PLIST" > "$PROFILE_CERT" 2>/dev/null; then
            EXISTING_IDENT=$(openssl x509 -inform DER -in "$PROFILE_CERT" -noout -fingerprint -sha1 2>/dev/null \
                | sed 's/^.*=//' | tr -d ':')
        fi
